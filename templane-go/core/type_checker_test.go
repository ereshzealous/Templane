...
func schemaOf(pairs ...any) *TypedSchema {
	fields := map[string]TemplaneField{}
	for i := 0; i < len(pairs); i += 3 {
		name := pairs[i].(string)
		t := pairs[i+1].(TemplaneFieldType)
		req := pairs[i+2].(bool)
		fields[name] = TemplaneField{Name: name, Type: t, Required: req}
	}
	return &TypedSchema{ID: "test", Fields: fields}
}

func str() TemplaneFieldType { return TemplaneFieldType{Kind: "string"} }
func num() TemplaneFieldType { return TemplaneFieldType{Kind: "number"} }
func boo() TemplaneFieldType { return TemplaneFieldType{Kind: "boolean"} }
...
func TestInvalidEnumValue(t *testing.T) {
	s := schemaOf("status", TemplaneFieldType{Kind: "enum", Values: []string{"active", "inactive"}}, true)
...
func TestNestedObjectTypeError(t *testing.T) {
	inner := TemplaneFieldType{Kind: "object", Fields: map[string]TemplaneField{
...
func TestListItemTypeMismatch(t *testing.T) {
	s := schemaOf("tags", TemplaneFieldType{Kind: "list", ItemType: &TemplaneFieldType{Kind: "string"}}, true)
