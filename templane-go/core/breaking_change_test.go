...
func TestEnumValueRemovedIsBreaking(t *testing.T) {
	old := schema("status", TemplaneFieldType{Kind: "enum", Values: []string{"active", "inactive", "pending"}}, true)
	nu := schema("status", TemplaneFieldType{Kind: "enum", Values: []string{"active", "inactive"}}, true)
	changes := DetectBreakingChanges(old, nu)
...
func TestEnumValueAddedNotBreaking(t *testing.T) {
	old := schema("status", TemplaneFieldType{Kind: "enum", Values: []string{"active"}}, true)
	nu := schema("status", TemplaneFieldType{Kind: "enum", Values: []string{"active", "inactive"}}, true)
	if changes := DetectBreakingChanges(old, nu); len(changes) != 0 {
		t.Errorf("got %+v", changes)
	}
...
func TestNestedObjectRecursion(t *testing.T) {
	innerOld := TemplaneFieldType{Kind: "object", Fields: map[string]TemplaneField{
		"city": {Name: "city", Type: str(), Required: true},
		"zip":  {Name: "zip", Type: str(), Required: false},
	}}
	innerNew := TemplaneFieldType{Kind: "object", Fields: map[string]TemplaneField{
		"city": {Name: "city", Type: str(), Required: true},
	}}
