...
// ParseResult is the outcome of parsing a TSP schema document.
...
func ParseSchema(yamlStr, schemaID string) ParseResult {
	...
	fields := make(map[string]TSPField, len(mapData))
...
func parseField(name string, def map[string]any) TSPField {
	typeStr := "string"
	if t, ok := def["type"].(string); ok {
		typeStr = t
	}
	required, _ := def["required"].(bool)
	return TSPField{Name: name, Type: parseType(typeStr, def), Required: required}
}

func parseType(typeStr string, def map[string]any) TSPFieldType {
	switch typeStr {
	case "string":
		return TSPFieldType{Kind: "string"}
	case "number":
		return TSPFieldType{Kind: "number"}
	case "boolean":
		return TSPFieldType{Kind: "boolean"}
	case "null":
		return TSPFieldType{Kind: "null"}
	case "enum":
		...
		return TSPFieldType{Kind: "enum", Values: values}
	case "list":
