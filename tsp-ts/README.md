...
TypeScript implementation of [TSP](../SPEC.md). Ships a full **Handlebars
integration** (`handlebars-tsp`) and the `xt` CLI (`render`, `check`, `test`,
`dev`, `build`).
...
// Render (throws TSPHandlebarsError on type errors)
...
```typescript
import { compile, TSPHandlebarsError } from './src/handlebars-tsp';

interface TSPTemplate {
  schema: TypedSchema;
  body: string;
  check(data: Record<string, unknown>): TypeCheckError[];
  render(data: Record<string, unknown>): string;  // throws TSPHandlebarsError
}

function compile(source: string, schemaId?: string): TSPTemplate;
```

`TSPHandlebarsError.errors` is the raw `TypeCheckError[]` array.
...
(10), handlebars-tsp (12), xt CLI (17).
...
TSP's AST and TIR types are discriminated unions. TypeScript's native
