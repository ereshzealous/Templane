...
# ---------------------------------------------------------------------------
# TemplaneFieldType hierarchy  (plain eq=True — no frozen, hashability not needed)
# ---------------------------------------------------------------------------
...
TemplaneFieldType = StringType | NumberType | BooleanType | NullType | EnumType | ListType | ObjectType
...
def templane_field_type_to_dict(t: TemplaneFieldType) -> dict:
    ...
    if isinstance(t, ListType):
        return {"kind": "list", "item_type": templane_field_type_to_dict(t.item_type)}
    if isinstance(t, ObjectType):
        return {
            "kind": "object",
            "fields": {k: templane_field_to_dict(v) for k, v in t.fields.items()},
        }
    raise ValueError(f"Unknown type: {t}")


def templane_field_type_from_dict(d: dict) -> TemplaneFieldType:
    kind = d["kind"]
    if kind == "string":
        return StringType()
    if kind == "number":
    ...
    if kind == "list":
        return ListType(item_type=templane_field_type_from_dict(d["item_type"]))
    if kind == "object":
...
def templane_field_from_dict(d: dict) -> TemplaneField:
    return TemplaneField(name=d["name"], type=templane_field_type_from_dict(d["type"]), required=d["required"])


def typed_schema_to_dict(s: TypedSchema) -> dict:
    return {"id": s.id, "fields": {k: templane_field_to_dict(v) for k, v in s.fields.items()}}
