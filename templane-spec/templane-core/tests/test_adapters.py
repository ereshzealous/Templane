from templane_core.html_adapter import render as html_render
from templane_core.yaml_adapter import render as yaml_render
...
def test_html_basic():
    ...
    assert result == "<!-- templane template_id=greeting schema_id=user -->\nHello Alice!"
...
def test_html_provenance_comment():
    ...
    result = html_render(tir)
    assert result.startswith("<!-- templane template_id=my-template schema_id=my-schema -->")
