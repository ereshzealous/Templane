from templane_core.breaking_change import detect, BreakingChange
from templane_core.models import (
    TypedSchema, TemplaneField, StringType, NumberType,
    EnumType, ObjectType,
)
...
def _s(*pairs):
    return TypedSchema(
        id="x",
        fields={n: TemplaneField(name=n, type=t, required=r) for n, t, r in pairs},
    )
...
def test_nested_object_recursion():
    inner_old = ObjectType(fields={
        "city": TemplaneField(name="city", type=StringType(), required=True),
        "zip":  TemplaneField(name="zip",  type=StringType(), required=False),
    })
