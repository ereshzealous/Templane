...
from templane_adapter_yaml.yaml_adapter import render as yaml_render
...
def test_html_basic():
    ...
    assert html_render(tir) == "<!-- templane template_id=greeting schema_id=user -->\nHello Alice!"
...
def test_html_provenance_comment():
    ...
    assert html_render(tir).startswith("<!-- templane template_id=my-template schema_id=my-schema -->")
...
def test_yaml_basic():
    tir = _tir(TIRTextNode("name: "), TIRExprNode("name", "Alice"),
               template_id="greeting", schema_id="user")
    assert yaml_render(tir) == "# templane template_id=greeting schema_id=user\nname: Alice"
...
def test_yaml_provenance_comment():
    tir = _tir(TIRTextNode("Hello"), template_id="my-template", schema_id="my-schema")
    assert yaml_render(tir).startswith("# templane template_id=my-template schema_id=my-schema")
